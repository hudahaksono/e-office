  ...
  OrgChart.prototype = {
    ...
    createNode: function (data) {
      ...
      var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (data[opts.nodeId] ? ' id="' + data[opts.nodeId] + '"' : '') + (data.parentId ? ' data-parent="' + data.parentId + '"' : '') + '>')
        ...
        console.log(opts.nodeTemplate);
      if (opts.nodeTemplate) {
        $nodeDiv.append(opts.nodeTemplate(data));
      } else {
          $nodeDiv.append('<div class="title">' + data[opts.nodeTitle] + '</div>')
              .append(typeof opts.nodeContent !== 'undefined' ? '<div class="content">' + (data[opts.markvalue] == 'x' ? ('<a style="font-style: italic;font-weight: bold;">&#9733; ' + data[opts.nodeContent] + '</a>' || '') : (data[opts.nodeContent] || '')) + '</div>' : '')
              .append(typeof opts.nodeHead !== 'undefined' ? '<div class="head">&nbsp;&nbsp;Status: <a style="font-style: italic;font-weight: bold;">' + (data[opts.nodeHead] || '') + '</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Position Id: ' + (data[opts.idbox] || '') + '</div>' : '');
      }
