...
namespace Surat.Codes.TataNaskah
{
    public class SuratEdaran
    {
        ...
        internal MemoryStream prepare(DraftSurat data, userIdentity usr, string pathx)
        {
            ...
            _cell = new Cell().Add(new Paragraph("SURAT EDARAN").SetMultipliedLeading(1.0f).SetFontSize(11).SetFont(fontBokos).SetTextAlignment(TextAlignment.CENTER)).SetBorder(Border.NO_BORDER);
            _cell.SetPaddingTop(6f).SetMargin(0f);
            _table.AddCell(_cell);
