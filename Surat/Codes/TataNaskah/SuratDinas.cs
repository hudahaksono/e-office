...
namespace Surat.Codes.TataNaskah
{
    public class SuratDinas
    {
        ...
        internal MemoryStream prepare(DraftSurat data, userIdentity usr, string pathx)
        {
            ...
            Cell celldalem = new Cell(1, 1).Add(new Paragraph("Yth.").AddStyle(styledalem).SetMultipliedLeading(.85f)).SetBorder(Border.NO_BORDER);
            ...
            {
                _cell = new Cell().Add(new Paragraph("Tembusan:").SetPaddingTop(5f).SetMultipliedLeading(1.0f).SetFontSize(11).SetFont(fontBokos)).SetBorder(Border.NO_BORDER);
                _table.AddCell(_cell);
            ...
            byte[] byteArray = kop.LokasiTanggal(ms.ToArray(), fontBokos,dokumen.TanggalSurat, lokasiditetapkan, pathx);
            byteArray = kop.polish(byteArray, fontBokos, dokumen.DraftCode, pathx);
